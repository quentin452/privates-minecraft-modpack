...
* new recommended jvm argument (the recommended jvm arguments are the same from several version ago, why in fact the old recommended jvm argument from V0.89.6 sometimes crashed with GC, it seems to me that the recommended jvm arguments are not the same (from several versions ago) had these crashes, so we will stay on these jvm args)
-XX:+UnlockExperimentalVMOptions -XX:+UseConcMarkSweepGC -XX:+UseFastAccessorMethods -XX:+UseCompressedOops -XX:+ParallelRefProcEnabled
