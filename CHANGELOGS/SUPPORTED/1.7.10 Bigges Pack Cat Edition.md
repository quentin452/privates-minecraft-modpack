...
* OptimizationsAndTweaks -> V1.17.3: mobs now flee/panic at the correct speed when hit, a survival-HUD crash is fixed, and duplicate enchantment ids auto-resolve instead of crashing the boot
* mod-director (FileDirector) bootstrapper -> 1.9.1-fork9: faster boot (mod info cached to disk) + a CurseForge scanner fix
* BetterQuesting -> 3.8.9-CAT: /bq_export enchants baseline + build fix
* LootBeamsBackport -> 1.0.4 (client-side)
* Essence of the Gods (eotg) V1.5.6 re-uploaded: fixes the survival-HUD crash (uncancelable event)
* ImmersiveEngineering -> 0.7.11-fork4: much faster world open (oredict lookup memoized, ~3x) + repaired build deps
